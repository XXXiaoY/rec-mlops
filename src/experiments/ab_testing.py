...
class StatisticalAnalyzer:
    ...
    def calculate_sample_size(
        effect_size: float, power: float = 0.95, alpha: float = 0.05, two_sided: bool = True
    ) -> int:
    ...
    def calculate_effect_size(
        control_mean: float, treatment_mean: float, pooled_std: float
    ) -> float:
    ...
    def calculate_confidence_interval(
        diff: float, std_error: float, confidence_level: float = 0.95
    ) -> Tuple[float, float]:
    ...
    def perform_statistical_test(
        control_data: np.ndarray, treatment_data: np.ndarray, test_type: str = "ttest"
    ) -> Tuple[float, float]:
...
class ABTestFramework:
    ...
    def assign_user_to_variant(self, experiment_id: str, user_id: int) -> str:
        ...
        variant = (
            "treatment" if np.random.random() < experiment["config"].traffic_split else "control"
        )
    ...
    def analyze_experiment(self, experiment_id: str) -> ExperimentResult:
        ...
        # Statistical test
        test_statistic, p_value = self.analyzer.perform_statistical_test(
    ...
    def run_experiment(
        self, experiment_id: str, duration_days: Optional[int] = None
    ) -> ExperimentResult:
    ...
    def list_experiments(self) -> Dict[str, Any]:
        ...
        for exp_id, result in self.experiment_results.items():
            experiments[exp_id] = {"status": "completed", "result": result}
...
async def demo_ab_testing():
    ...
    print(
        f"95% Confidence interval: [{result.confidence_interval[0]:.6f}, {result.confidence_interval[1]:.6f}]"
    )
