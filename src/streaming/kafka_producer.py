...
class KafkaProducer:
    ...
    async def send_user_interaction(self, interaction: Dict[str, Any]) -> bool:
        ...
        return await self.send_message(
            "user_interactions", interaction, key=str(interaction.get("user_id"))
        )
    ...
    async def send_model_update(self, model_data: Dict[str, Any]) -> bool:
        ...
        return await self.send_message("model_updates", model_data, key=model_data.get("model_id"))
...
async def example_consumer_usage():
    ...
    consumer = KafkaConsumer(config, ["user_interactions", "recommendations_served"])
